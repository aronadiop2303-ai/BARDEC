...
- 🔄 Paiement réel (Wave/Orange Money/MTN MoMo) : **préparation d'architecture uniquement, aucun flux d'argent réel possible**. Flux actif aujourd'hui = preuve de paiement manuelle (`checkout.tsx` → `payment_proof_url` → vérification admin), pas d'appel API. Table `payment_providers` (registre générique fournisseur, patron `delivery_partners`) proposée mais pas encore appliquée en base — attend validation d'Arona sur le SQL. Edge Function stub `payment-gateway` écrite (toujours `501 not_implemented`, `REAL_PAYMENTS_ENABLED=false` en dur, aucune clé/endpoint réel) mais pas encore déployée. Voir BUGS.md pour le détail et ce qui manque avant un vrai lancement (clés API, KYB, webhook, remboursements).
