...
- 🔄 Paiement réel : preuve manuelle (Wave/Orange Money/MTN MoMo, `checkout.tsx` → `payment_proof_url` → vérification admin) reste le seul flux appelé par l'app — inchangé. **PayDunya (agrégateur Wave/Orange Money/Free Money/carte) intégré réellement en mode SANDBOX uniquement** (`payment-gateway` + nouvelle `payment-gateway-webhook`, testé bout-en-bout le 9 sept avec de vraies clés de test PayDunya — voir BUGS.md). Structurellement, aucun vrai argent ne peut être déplacé : NINEA non validé, `PAYDUNYA_MODE=test`, `checkout.tsx` n'appelle pas encore ces fonctions. Tout autre `provider_code` (`wave`/`orange_money`/`mtn_momo`/`bank_transfer`/`card`/`paypal`) reste sur le stub `501 not_implemented` historique. Voir BUGS.md pour le détail complet et ce qui manque avant le Live (NINEA, bascule manuelle de `PAYDUNYA_MODE`, vraies clés de production, branchement de `checkout.tsx`, remboursements).
- 🔄 Crédit Net30 B2B : conception validée par Arona, **backend + frontend + écran admin branchés**. Triggers `apply_net30_charge`/`apply_net30_settlement` sur `orders` débitent/créditent `companies.net30_balance`. `AuthContext` lit désormais `credit_limit`/`net30_balance`/`is_approved` depuis `companies` (pas `users`). `net30` réactivé dans `checkout.tsx`, gated à `company.is_approved && credit_limit > 0`. Badge de dépassement côté file d'approbation (`orders.tsx`), non bloquant. Onglet admin "Sociétés B2B" (`admin.tsx`) : approuver une société + fixer sa `credit_limit`. **Reste à faire** : tests empiriques sur téléphone. Voir BUGS.md pour le détail complet.
