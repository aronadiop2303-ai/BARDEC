...
- 🔄 Crédit Net30 B2B : conception validée par Arona, **triggers backend appliqués et testés en base** (`apply_net30_charge`/`apply_net30_settlement` sur `orders`, débitent/créditent `companies.net30_balance`). `net30` reste `available:false` côté `checkout.tsx` — le frontend (lecture `user.creditLimit/creditBalance` depuis `companies`, réactivation du payment method, UI admin pour `credit_limit`, signal de dépassement côté approbateur) n'est pas encore branché. Voir BUGS.md pour le détail et la liste précise de ce qu'il reste à faire.
