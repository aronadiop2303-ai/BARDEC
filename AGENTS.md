...
- 🔄 Paiement réel (Wave/Orange Money/MTN MoMo) : **préparation d'architecture uniquement, aucun flux d'argent réel possible**. Flux actif aujourd'hui = preuve de paiement manuelle (`checkout.tsx` → `payment_proof_url` → vérification admin), pas d'appel API. Table `payment_providers` (registre générique fournisseur, patron `delivery_partners`) appliquée en production (validée par Arona). Edge Function `payment-gateway` déployée — toujours `501 not_implemented`, `REAL_PAYMENTS_ENABLED=false` en dur, aucune clé/endpoint réel, non appelée par `checkout.tsx`. Voir BUGS.md pour le détail et ce qui manque avant un vrai lancement (clés API, KYB, webhook, remboursements).
- 🔄 Crédit Net30 B2B : conception validée par Arona, **backend + frontend branchés**. Triggers `apply_net30_charge`/`apply_net30_settlement` sur `orders` débitent/créditent `companies.net30_balance`. `AuthContext` lit désormais `credit_limit`/`net30_balance`/`is_approved` depuis `companies` (pas `users`). `net30` réactivé dans `checkout.tsx`, gated à `company.is_approved && credit_limit > 0`. Badge de dépassement côté file d'approbation (`orders.tsx`), non bloquant. **Reste à faire** : écran admin pour éditer `credit_limit` par société (aucun écran "Sociétés" n'existe encore, plus gros qu'un simple câblage), tests empiriques sur téléphone. Voir BUGS.md pour le détail complet.
